...
class InteractionTypes extends StatefulWidget {
  const InteractionTypes({Key key}) : super(key: key);

  @override
  _InteractionTypesState createState() => _InteractionTypesState();
}

class _InteractionTypesState extends State<InteractionTypes> with AutomaticKeepAliveClientMixin {
  ...
  List<Widget> getTabs(SampleListModel model) {
    ...
    for (int i = 0; i <
        model.controlList[model.selectedIndex].subItemList.length; i++) {
      tabs.add(
          Tab(text: model.controlList[model.selectedIndex].subItemList[i][0]
              .category));
    }
    return tabs;
  }
