...
import '../helper/predefined_categories.dart';
...
Future<void> writeTransactionalDetails(
  ...
  if (goalsSheet.rows.isNotEmpty) {
    for (int rowIndex = 1; rowIndex < goalsSheet.rows.length; rowIndex++) {
      goalsSheet.removeRow(rowIndex);
    }
  }
...
List<TransactionalDetails> readAllTransactionDetailCollections(
  UserDetails userDetails, {
  bool isNewUser = false,
}) {
  ...
  final List<TransactionalDetails> data = transactionalData.data;

  if (transactionalData.data.isEmpty) {
    data.add(
      TransactionalDetails(
    ...
    allTransactionalDetails.addAll(data);
  } else {
    allTransactionalDetails.addAll(data);
  }

  return allTransactionalDetails;
}
...
Future<void> writeTransactionalDetailsToFile(
  ...
  final TransactionalDetails currentDetails = transactionalData.data[0];
